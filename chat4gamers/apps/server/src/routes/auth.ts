...
const auth = new Hono()
const pendingChallenges = new Map<PublicKey, { nonce: string; expiresAt: number }>()
...
auth.post('/verify', async (c) => {
  const { publicKey: publicKeyBase64, signature: signatureBase64 } = await c.req.json()
  ...
  try {
    ...
    isValid = await crypto.subtle.verify('Ed25519', cryptoKey, signatureBuffer, challengeBuffer)
  } catch {
  ...
  const user = db.select().from(users).where(eq(users.publicKey, publicKeyBase64)).get()
  ...
  const sessionToken = Buffer.from(crypto.getRandomValues(new Uint8Array(32))).toString('base64url')
