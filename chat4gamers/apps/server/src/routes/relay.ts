...
router.post('/relay/dm', async (c) => {
  try {
    ...
    const payload = Buffer.from(
      `dm:${recipientId}:${timestamp}:${content}`,
      'utf8'
    )
    ...
    console.error('POST /relay/dm error:', err)
    return c.json(
...
router.get('/relay/dm-history', async (c) => {
  ...
  if (!me || !other)
    return c.json({ error: 'me and with query params required' }, 400)
  ...
  try {
    const history = await db
      ...
      .where(
        or(
          and(
            eq(directMessages.senderId, me),
            eq(directMessages.recipientId, other)
          ),
          and(
            eq(directMessages.senderId, other),
            eq(directMessages.recipientId, me)
          )
        )
      )
