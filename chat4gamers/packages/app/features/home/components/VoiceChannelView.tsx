  ...
  return (
    <YStack flex={1} alignItems="center" justifyContent="center" gap="$6" p="$6">
      <XStack gap="$2" alignItems="center">
        ...
        <Text fontSize="$6" color="$color10" fontWeight="600">{channelId}</Text>
      </XStack>

      {participants.length === 0 ? (
        <Text color="$color10" fontSize="$3">No one is here yet — join from the sidebar</Text>
      ) : (
        <XStack gap="$4" flexWrap="wrap" justifyContent="center">
          {participants.map(name => (
            <YStack key={name} alignItems="center" gap="$2" width={80}>
              ...
              <Text fontSize="$2" color="$color" numberOfLines={1} textAlign="center">{name}</Text>
            </YStack>
          ))}
        </XStack>
      )}
