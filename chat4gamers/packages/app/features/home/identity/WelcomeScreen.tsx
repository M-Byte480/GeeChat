...
import { YStack, XStack, Text, Input, Button, Paragraph } from '@my/ui'
import type { Identity, IdentityFile } from './types'
import { generateIdentity, decryptIdentity, serializeForStorage } from './crypto'
...
export function WelcomeScreen({ onIdentityReady }: Props) {
  ...
  const handleImportClick = async () => {
    ...
    try {
      ...
      setError(e.message || 'Could not read identity file')
    }
  }
  ...
  const handleCreate = async () => {
    ...
    setLoading(true)
    try {
      const { file, identity } = await generateIdentity(username.trim(), pfp, passphrase)
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      setError(e.message || 'Failed to decrypt identity')
      setLoading(false)
    }
  }
  ...
  return (
    ...
    >
      ...
      {error && (
        <Paragraph color="$red10" fontSize="$3" textAlign="center" maxWidth={320}>
          {error}
        </Paragraph>
      )}
    </YStack>
