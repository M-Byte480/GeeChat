...
export function ServerListComponent({ servers, activeServerId, onSelectServer, serverContextOptions }: Props) {
  return (
    <YStack gap="$2" alignItems="center" paddingTop="$2">
      {servers.map((server) => {
        ...
        const initials = server.name
          ...
          .join('')
        const options = serverContextOptions?.(server) ?? []

        return (
          <ContextMenu key={server.id} options={options}>
